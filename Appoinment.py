        ...
        if mob[0]['mobile'] == 1:
            ...
            if mobile[0]['mobile'] == 1 and doctorid[0]['doctor_id'] == 1 and businessid[0]['business_id'] == 1:
                    ...
                    b = json.loads(dbget("select * from new.timing where doctor_id = '" + str(docidval)+"' and business_id = '"+str(bus_id)+"'\
                                        and day = '"+str(x)+"' order by session"))
                    print('end_timing',b)
                ...
                return (json.dumps({"Message": "Please Give a valid Data ", "Message_Code": "PGVD","Service_Status": "Failure"},indent=4))
...
def updatetoken(request):
    try:
        ...
        if app_id[0]['appointment_id']==1:
                dbput("update new.appointment set token_status='" + str(d['token_status']) + "',reason='" + str(d['reason']) + "'  where app_id='" + str(d['appointment_id']) + "'")
