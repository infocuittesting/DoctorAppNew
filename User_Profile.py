...
def myappointments(request):
    try:
        ...
        output = json.loads(dbget("select new.appointment.app_id, new.doctor_profile.doctor_name,new.business_profile.business_name,\
                                    ...
                                    new.appointment.business_date as appointment_date,new.appointment.reason,new.appointment.token_no,new.appointment.doctor_id,new.appointment.business_id\
        ...
        for i in output:
            if datetime.datetime.strptime(i['appointment_date'], '%Y-%m-%d').date()==today:
                i['flag']='today'
                i['image_url']=""
            elif datetime.datetime.strptime(i['appointment_date'], '%Y-%m-%d').date()<today:
                i['flag']='past'
                i['image_url']=""
            else:
                i['flag']='future'
